...
这是一个独立、高性能的 Golang 服务，**作为被动连接的 WebSocket 服务端**，接受外部客户端的信号推送，自动向多个下游（如各大加密货币交易所的 API）发起 HTTP 下单请求。

支持两种模式：
- **WS 服务端模式（默认）**：监听本地 WebSocket 端口，等待外部客户端连接后发信号下单。
- **上游 WS 客户端模式（可选）**：主动连接上游信号源（旧模式，通过配置启用）。

## 核心特性

- **被动接入**：作为 WS 服务端运行，暴露 `ws://host:port/ws` 端点，外部推送即可触发下单。
- **多客户端管理**：支持多个 WS 客户端同时连接，实时显示连接数。
- **接入密钥验证**：可选配置 `key` 参数，防止未授权接入。
- **心跳保活**：自动 Ping/Pong 心跳管理，连接断开自动清理。
- **多任务并发执行**：收到一条信号，可同时触发多个下游 API 任务。
...
- **优雅的前端面板**：提供现代化的 Web 控制台，实时监控运行状态。
...
- **WS 路径**：服务端监听路径，默认 `/ws`，接入地址为 `ws://your-host:9946/ws`
- **接入密钥**：可选，客户端需通过 `?key=xxx` 参数传入才允许连接
- **启用**：开启后 WS 服务端开始接受连接
- **应用跳过信号逻辑**：开启后 WS 服务端收到的信号也会应用各任务"跳过前 N 次信号"规则
- **接入地址**：自动生成的完整 WS 地址，支持一键复制

外部客户端连接后发送 JSON 信号即可触发下单：
```json
{"type":"signal","orderID":123,"action":"buy","symbol":"BTCUSDT","timestamp":"2026-07-08 15:04:05"}
```

### 2. 上游 WebSocket（主动连接，旧模式）
...
- `{{amount}}` / `{{unit}}` / `{{symbol}}` / `{{tickerType}}`：也会从信号中提取并替换。
...
在任务的最底部，有一个 **允许的交易对 (Allowed Symbols)** 配置项。
- 留空：该任务将接收并处理所有交易对的信号。
- 填入 `BTCUSDT,ETHUSDT`：只有当信号里的 `symbol` 是 BTCUSDT 或 ETHUSDT 时，该任务才会被触发下单。其他币种信号将被直接跳过。
...
外部客户端连接到 WS 服务端后，发送以下 JSON 消息即可触发下单：
...
| `amount` | - | 下单金额 |
| `unit` | - | 时间单位，如 `TEN_MINUTE` |
| `tickerType` | - | Ticker 类型 |
| `orderID` | - | 订单追踪 ID |
| `timestamp` | - | 时间戳 |
